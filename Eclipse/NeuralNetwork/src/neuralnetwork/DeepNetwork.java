...
public class DeepNetwork extends Network {
	...
	public static float weightMult = 0.0013f;
    ...
    */
	...
	public void train(ArrayList<Example> trainExamples, long timeMS) {
		...
		train(trainExamples, timeMS, 128, false);
	}
	
	public void train(ArrayList<Example> trainExamples, int nbreSimUnits, long timeMS) {
		
		train(trainExamples, timeMS, nbreSimUnits, false);
	}
	
	public void train(ArrayList<Example> trainExamples, long timeMS, boolean drawDebug) {
		
		train(trainExamples, timeMS, 128, drawDebug);
	}
	
	public void train(ArrayList<Example> trainExamples, long timeMS, int nbreSimUnits, boolean drawDebug) {
		...
		while (System.currentTimeMillis() - time < timeMS) {
			...
			while(ended == false) {
				...
				for(SimulationUnit su : childs)
					if(su.t.getState() == Thread.State.RUNNABLE || su.t.getState() == Thread.State.BLOCKED) {
						ended = false;
					}
	...
	public DeepNetwork createChild(int nbreModifs) {
		...
		for(ArrayList<Neuron> couche : dn.network) {
			...
			for(Neuron neuron : couche)
				neuron.modifyWeights(adjustPrecision/inputsNumber, nbreModifs);
	...
	public DeepNetwork createChild(float modifiedProportion) {
		...
		for(ArrayList<Neuron> couche : dn.network) {
			...
			for(Neuron neuron : couche)
				neuron.modifyWeights(adjustPrecision/inputsNumber, Math.round(modifiedProportion*inputsNumber));
	...
	private static ArrayList<ArrayList<Neuron>> randomNetwork(int[] nbNeurones) {
		...
		for (int i = 1; i < nbNeurones.length; i++) {
			...
			for (int y = 0; y < nbNeurones[i]; y++)
				couche.add(randomNeuron(nbNeurones[i - 1], weightMult/nbNeurones[i - 1]));
