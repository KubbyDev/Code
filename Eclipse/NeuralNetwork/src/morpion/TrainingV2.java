...
//C'est la V4 du puissance 4 adaptée au morpion
public class TrainingV2 {

	public static long timeMS = 10000000;
	public static float adjustPrecision = 2.0f;
	public static int modificationsNumber = 50;
	public static int[] nbreNeurons = {19,20,20,9};
	public static boolean stopRequest = false;
	public static int displayOneOf = 20; //Proportion des generations affichees (10: 0.1; 100: 0.01)
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < 5; i++) {
			...
			if(BiologicalNetwork.exists(2200+i))
				networks.add(new BiologicalNetwork(2200+i));
			else
		...
		while (System.currentTimeMillis() - time < timeMS && !stopRequest) {
			...
			networks = getBestNetworks(networks, nbreGenerations%displayOneOf == 0);
	...
	private static ArrayList<BiologicalNetwork> getBestNetworks(ArrayList<BiologicalNetwork> list, boolean displayResults) {
		...
		//The max possible value is 294 because each network plays twice against 49 networks, and a victoiry is 3 points
		int maxValue = 294;
		int newMaxValue = 0;
	...
	private static int getResult(BiologicalNetwork network, ArrayList<BiologicalNetwork> list) {
		...
		for(BiologicalNetwork dn : list) {
			...
			if(dn != network) {
				...
				//On ajoute 3 en cas de victoire, 0: defaite, 2: egalite
				
				result += Math.min(3, (getWinner(network, dn) +1)*2);
				
				//On inverse le resultat de la partie puisque comme le reseau qu'on teste ne commence pas, on veut une defaite
				result += Math.min(3, (getWinner(dn, network)*-1 +1)*2);
