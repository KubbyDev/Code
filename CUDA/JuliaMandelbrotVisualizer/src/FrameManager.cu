...
Frame* initFrame(int width, int height, int posx, int posy, int additionnalDataCount) {
    ...
    frame->width = 2;
    frame->height = (frame->width) * (VAR_TYPE)height/width;
    frame->topLeftX = -1;
    ...
    if(additionnalDataCount > 0) {
        VAR_TYPE* data;
        cudaMallocManaged(&data, sizeof(VAR_TYPE)*additionnalDataCount);
...
void zoom(Frame* frame, int newCenterX, int newCenterY, VAR_TYPE zoomFactor) {
    ...
    frame->topLeftX +=
        ((VAR_TYPE)newCenterX/(frame->widthPixels))*(frame->width) //Position of the new center in real plane
        - (frame->width)/2 *zoomFactor;                            //Position of the new top left corner in real plane
    frame->topLeftY -=
        ((VAR_TYPE)newCenterY/(frame->heightPixels))*(frame->height)
