/*
 * Shamelessly stolen from here: http://derekmolloy.ie/kernel-gpio-programming-buttons-and-leds/#Example_3_Enhanced_LED_GPIO_Driver_LKM
*/
...
static ssize_t blinkPeriod_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf){
   return sprintf(buf, "%d\n", blinkPeriod);
}

/** @brief A callback function to store the LED period value */
static ssize_t blinkPeriod_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count){
 ...
 */
static struct kobj_attribute period_attr = __ATTR_RW(blinkPeriod);
...
static int __init ebbLED_init(void){
   ...
   sprintf(ledName, "led%d", gpioLED);      // Create the gpio115 name for /sys/kernel/ebb/led17
   ...
   // add the attributes to /sys/ebb/ -- for example, /sys/kernel/ebb/led17/ledOn
