/*
 * Shamelessly stolen from here: http://derekmolloy.ie/kernel-gpio-programming-buttons-and-leds/
*/
 ...
 * The sysfs entry appears at /sys/kernel/ebb/gpio27
...
#include <linux/time64.h>       // Using the clock to measure time between button presses
...
MODULE_PARM_DESC(gpioButton, " GPIO Button number (default=27)");  ///< parameter description
...
static char   objectname[] = "button";      ///< Kernel object name > Appears in /sys/kernel/ebb
...
static struct timespec64 ts_last, ts_current, ts_diff;  ///< timespecs from linux/time.h (has nano precision)
...
static ssize_t lastTime_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf){
   return sprintf(buf, "%.2u:%.2u:%.2u:%.9lu \n", ((uint32_t)ts_last.tv_sec/3600)%24,
          ((uint32_t)ts_last.tv_sec/60) % 60, (uint32_t)ts_last.tv_sec % 60, ts_last.tv_nsec );
}
 ...
 *
 *  The __ATTR caused compilation problems so I replaced it by the newer __ATTR_RW macro
 */
static struct kobj_attribute count_attr = __ATTR_RW(numberPresses);
static struct kobj_attribute debounce_attr = __ATTR_RW(isDebounce);
...
static int __init ebbButton_init(void){
   ...
   ebb_kobj = kobject_create_and_add("ebb", kernel_kobj); // kernel_kobj points to /sys/kernel
   if(!ebb_kobj){
   ...
   ktime_get_real_ts64(&ts_last);                       // set the last time to be the current time
   ts_diff = timespec64_sub(ts_last, ts_last);          // set the initial time difference to be 0

   // Going to set up the LED. It is a GPIO in output mode and will be on by default
   ledOn = true;
   gpio_request(gpioLED, "sysfs");          // gpioLED is hardcoded to 17, request it
...
static irq_handler_t ebbgpio_irq_handler(unsigned int irq, void *dev_id, struct pt_regs *regs){
   ...
   ktime_get_real_ts64(&ts_current);    // Get the current time as ts_current
   ts_diff = timespec64_sub(ts_current, ts_last);   // Determine the time difference between last 2 presses
